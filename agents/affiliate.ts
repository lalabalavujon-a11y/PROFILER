...
export async function affiliateNode(state: AffiliateState) {
  ...
  const affiliateId =
    packet.host?.affiliateId || generateAffiliateId(packet.host?.name);
  ...
  const affiliateResources = await generateAffiliateResources(
    packet,
    affiliateLink
  );
...
function calculateCommissionStructure(
  ...
  const bumpCommission = bumpEnabled
    ? Math.round((bumpPrice * commissionPct) / 100)
    : 0;
...
The creator, ${
          packet.host?.name || "the expert"
        }, is hosting an exclusive training session for ${
          packet.audience?.industry || "business"
        } owners.
...
This isn't available to everyone - only ${
          packet.audience?.size === "enterprise"
            ? "enterprise leaders"
...
${
  packet.host?.name || "Industry expert"
} is sharing the exact blueprint in a free training session.

If you're serious about scaling your lead generation, this is worth your time.

Details: ${affiliateLink}

#LeadGeneration #AI #BusinessGrowth #${
          packet.audience?.industry?.replace(/\s+/g, "") || "Business"
        }`,
      },
      {
        platform: "Facebook",
    ...
    bannerAds: [
      {
        ...
        description: `Join ${packet.host?.name || "the expert"} for exclusive ${
          packet.audience?.industry || "business"
        } training`,
...
"If you're struggling to find qualified leads for your ${
          packet.audience?.industry || "business"
...
[SOLUTION - 15 seconds]
"${
          packet.host?.name || "This expert"
        } has cracked the code on AI-powered lead intelligence. The results? 300% more qualified leads, 50% shorter sales cycles."

[PROOF - 10 seconds]
"Fortune 500 companies are already using this system. Now it's available to ${
          packet.audience?.size === "enterprise"
            ? "enterprise leaders"
            : "business owners"
        } like you."
