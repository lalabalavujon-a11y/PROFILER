...
export class GoogleSlidesClient {
  ...
  async createPresentation(config: PresentationConfig): Promise<GoogleSlidesPresentation> {
    try {
  ...
  private async addContentToSlides(presentationId: string, content: SlideContent[]) {
    const requests = [];
    ...
    if (requests.length > 0) {
      try {
        ...
        console.warn('Some content updates failed, continuing with presentation creation');
      }
    }
  }

  private async applyBranding(presentationId: string, branding: { colors: string[]; logoUrl?: string }) {
    const requests = [];
