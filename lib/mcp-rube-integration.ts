...
export class MCPRubeClient {
  ...
  private async makeRequest(endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET', data?: any) {
    ...
    try {
      ...
      console.error(`Rube API error (${method} ${endpoint}):`, error.response?.data || error.message);
      throw error;
    }
  }

  async createLeadNurturingWorkflow(eventId: string, segments: any[]): Promise<RubeWorkflow> {
  ...
  private async generateEmailSequence(segment: any): Promise<Array<{ subject: string; content: string; delay: string }>> {
  ...
  async createCRMIntegration(eventId: string, crmType: 'hubspot' | 'salesforce' | 'pipedrive'): Promise<RubeAction> {
  ...
  async createSocialMediaCampaign(eventId: string, segments: any[]): Promise<RubeAction[]> {
    const campaigns: RubeAction[] = [];

    for (const segment of segments.slice(0, 3)) { // Limit to top 3 segments
      const campaign: RubeAction = {
        id: `social_campaign_${segment.name.toLowerCase().replace(/\s+/g, '_')}`,
          ...
          content: {
            linkedin: {
              ...
              targetAudience: {
                ...
                companySize: segment.name.includes('Enterprise') ? '500+' : '50-500',
              },
            },
            facebook: {
  ...
  async getWorkflowAnalytics(workflowId: string): Promise<{
    ...
    try {
      const analytics = await this.makeRequest(`/workflows/${workflowId}/analytics`);
      return analytics;
    } catch (error) {
