...
export async function generateLeadSegments(
  ...
  const { minSegmentSize = 5, maxSegments = 8 } = config;
  ...
  const consultingLeads = scoredLeads.filter(
    (lead) =>
      ["Consulting", "Agency"].includes(lead.industry) && lead.score > 0.5
  );
  ...
  const recentEngagedLeads = scoredLeads.filter((lead) => {
...
export function getSegmentInsights(segments: LeadSegment[]) {
  const totalLeads = segments.reduce(
    (sum, segment) => sum + segment.leads.length,
    0
  );
  const weightedConversionRate =
    segments.reduce(
      (sum, segment) =>
        sum + segment.estimatedConversionRate * segment.leads.length,
      0
    ) / totalLeads;
